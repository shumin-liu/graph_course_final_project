...
).update_layout(coloraxis_showscale = False, title_x = 0.5)
...
hdi_gdp_combined = pd.merge(hdi_long, gdp_pcap_long, on = ["country", "geo", "year"], how = "inner").query("year >= 2000")
...
hdi_gdp_scatter = px.scatter(hdi_gdp_combined,
  ...
  labels = {"human_development_index": "Human Development Index", 
            "gdp_per_capita": "GDP per Capita",
            "year": "Year"},
  title = "Relationship between Human Development Index and GDP per Capita from 2000 to 2023",
  template = "simple_white"
  )

hdi_gdp_scatter = hdi_gdp_scatter.update_layout(
  title_x = 0.5,
  margin = dict(l = 5, r = 5, t = 40, b = 120),
  showlegend = False
)

# Numbers for the value card
# Number of countries in the data
number_of_countries = hdi_gdp_combined.query("year == 2023")["country"].nunique()

# Calculate the average HDI in 2023
avg_hdi_2023 = hdi_gdp_combined.query("year == 2023")["human_development_index"].mean()

# Calculate the average GDP per Capita in 2023
avg_gdp_2023 = hdi_gdp_combined.query("year == 2023")["gdp_per_capita"].mean()
```
...
According to [the documentation of GDP per Capita](https://www.gapminder.org/data/documentation/gd001/), data between 1990 and 2022 are identical to [the GDP per capita data from the World Bank](https://data.worldbank.org/indicator/NY.GDP.PCAP.PP.KD), published in April 2024 in their World Development Indicators. Data prior to 1990 are from other sources. It is observed that data from 2022 onwards are also available. Therefore, it is assumed that data prior to 2022 are the actual historical data while data beyond 2022 are the future projection.
...
# Relationship between HDI and GDP per Capital

## Row 3.1 {height=20%}
::: {.valuebox icon="globe" color="#1E90FF" title="Number of countries"}
`{python} number_of_countries`
:::
::: {.valuebox icon="heart" color="#4682B4" title="Average HDI in 2023"}
`{python} str(round(avg_hdi_2023, 2))`
:::
::: {.valuebox icon="graph-up" color="#87CEFA" title="Average GDP per Capita in 2023"}
`{python} str(round(avg_gdp_2023))`
:::

## Row 3.2 {height=65%}

### Column 3.2.1 {width="50%"}
```{python}
hdi_gdp_scatter
```

### Column 3.2.2 {width="50%"}

```{python}
itables.show(
    hdi_gdp_combined.drop("year_category", axis = 1).reset_index(drop = True)
)
```
...
The scatter plot above shows a positive correlation between Human Development Index and GDP per Capita among 191 countries. 

This patterns are observed over time. In general, countries with higher GDP per capita tend to have higher Human Development Index.
:::